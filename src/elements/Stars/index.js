...
function Stars() {
    // consts
    const columns = ['full_title', 'singer', 'date', 'year', 'genre_refined', 'rank',
                        'lyric_rat', 'billboard_200', 'billboard_hot100'];
    ...
    const sounds = [0,
        new Audio(nobody), new Audio(gangnamStyle), new Audio(gentleman), new Audio(ddudu), new Audio(howYouLikeThat),
        new Audio(lovesickGirls), new Audio(thatThat), new Audio(dna), new Audio(fakeLove), new Audio(boyWithLuv),
        new Audio(onBTS), new Audio(dynamite), new Audio(lifeGoesOn), new Audio(butter)];
    ...
    // handler
    const handleResize = () => {
        if (Math.abs(width - window.innerWidth) >= 30 || Math.abs(height - window.innerHeight) >= 30) {
    ...
    useEffect(()=>{
        ...
        d3.csv(dataset).then(function(rawData) {
            let updateSongs = [], alreadyAppended = [];
            let id = 0;
    ...
    useEffect(()=>{
        ...
        d3.csv(billboard_in).then(function(rawData) {
            let updateBillboards = [], updateIndex = [];
            let id = 1, bts = 8, listid = 1;
            const bColumns = ['title', 'date', 'singer', 'lyric_rat'];
            for (let row of rawData) {
                let newSong = { 'id' : listid };
                for (let col of bColumns)
                    newSong[col] = row[col];
                if (row['singer'] === '방탄소년단') {
    ...
    useEffect(()=>{
        // draw billboard list
        const billboardList = d3.select(billboardRef.current)
        ...
        const title = billboardList.append("div")
            .style("width", "65vh")
            ...
            .attr("transform", `translate(${height * chartSize / 4}, ${height * 0.95})`);
        title.append("text")
            .text("billboard hot 100");
        
        const songList = billboardList.append("div")
            .style("width", "66vh")
            .style("height", "30vh")
            .style("position", "absolute")
            .style("display", "flex")
        ...
        if (numImgs.length > 0 && billboards.length > 0) {
            const leftColumn = songList.append("div")
                .style("margin-top", "1vh");
            let id = 0;
            for (let i = 0; i < 14; i ++) {
                if (billboards[i]['singer'] === "방탄소년단")
                    continue;
                const unitElem = leftColumn.append("div")
            ...
            const rightColumn = songList.append("div")
                .style("margin-top", "1vh");
            for (let i = 0; i < 14; i ++) {
                if (billboards[i]['singer'] !== "방탄소년단")
                    continue;
                const unitElem = rightColumn.append("div")
                    .attr("id", `billboard-${id+1}`)
                    .style("font-size", "1.2vh")
                    .style("width", "32vh")
                    .style("display", "flex")
    ...
    useEffect(()=>{
                ...
                arcs.append("path")
                    ...
                    .attr("d", () => {
                        return d3.arc()({
                            ...
                            startAngle: radian((pivot['yearFrom'] - startYear) * (180 / (2022-startYear)) - 90),
                            endAngle: radian((pivot['yearTo'] - startYear) * (180 / (2022-startYear)) - 90)
                        });
                    })
                    .attr("fill" , "#444444");
            }

            if ((year === pivot['yearFrom'] && year !== 2008) || year === 2012 || year === 2022) {
                let tickRadius = height * 1.8 * gridScale(pivot['stepTo']) / chartSize;
                ...
                ticks.append("text")
                    .text(year)
                    .attr("transform", `translate(${tickX}, ${tickY}), rotate(${tickAngle <= -90? tickAngle + 180 : tickAngle})`);
            }
        }

        let lines = grids.append("g").attr("class","grid-line-group");
        for (let deg = 0, year = startYear; deg <= 180; deg += 180 / (2022 - startYear), year += 1) {
            lines.append("line")
                .attr("class", `line-${deg}`)
                .attr("x1", height * 1.8 * gridScale(1) / chartSize * Math.cos(radian(deg - 180)))
                .attr("y1", height * 1.8 * gridScale(1) / chartSize * Math.sin(radian(deg - 180)))
                .attr("x2", height * 1.8 * gridScale(lineGridLength(year)) / chartSize * Math.cos(radian(deg - 180)))
                .attr("y2", height * 1.8 * gridScale(lineGridLength(year)) / chartSize * Math.sin(radian(deg - 180)))
                .style("stroke", "#444444");
        }
        ...
        for (let step = 1; step <= 5; step += 1) {
            ticks.append("text")
                ...
                .attr("text-anchor", "end")
                .attr("transform", `translate(${height * 1.8 * gridScale(step) / chartSize - height * 0.01}, ${height * 0.02})`);
        }
    }, [width, height]);
    ...
    useEffect(()=>{
        ...
        const gradTransparent = defs.append('linearGradient')
            .attr('id', 'traceGrad-transparent')
            .attr('r', '80%');
        gradTransparent.append('stop')
            .attr('offset', '0%')
            .attr('stop-color', 'transparent');
        gradTransparent.append('stop')
            .attr('offset', '47%')
        ...
        const rankedSongs = songs.filter((song) => Number(song['rank']) <= 30 && song['overlap'] == false);
        ...
        for (let year = startYear; year <= 2022; year += 1) {
            ...
            for (let {id, date, full_title, singer, lyric_rat, billboard_200, billboard_hot100, count} of thisYear) {
                if (Number(date.slice(0, 4)) < 1991) continue;

                const rat = Number(lyric_rat);
                const launch = [Number(date.slice(0, 4)), Number(date.slice(6, 8)), Number(date.slice(10, 12))];
                ...
                const billboard = billboard_200 === 'TRUE' || billboard_hot100 === 'TRUE';
                const arcWidth = height * 1.8 / 500000 / (billboard? 0.8 : 4.5);
                
                const startRad = radian(unitDegree - (180 / (2022 - startYear)) * count);
                const endRad = radian(unitDegree + (180 / (2022 - startYear)) * count);

                const headSize = height * 1.8 / 500 / (billboard? 1 : 2);
                const dotDeg = radian(unitDegree + (180 / (2022 - startYear)) * count - 90);
                ...
                if (selected > 0) {
                    let selectedTitle = '', selectedSinger = '';
                    ...
                    const drawPath = trace.append("path")
                        .attr("class", `${year}-${full_title}-trace`)
                        .attr("d", d3.arc()
                            .innerRadius(height * 1.8 * r / chartSize)
                            .outerRadius(height * 1.8 * (r + (full_title.includes(selectedTitle)? arcWidth * 2 : arcWidth)) / chartSize)
                            .cornerRadius(30)
                            .startAngle(startRad)
                            .endAngle(endRad)
                        );
                    ...
                    if (billboard)
                        drawPath.style("filter" , "url(#glow)");

                    head.append("circle")
                        .attr("class", `${year}-${full_title}-head`)
                        .attr("r", headSize)
                        .attr("cx", Math.cos(dotDeg) * height * 1.8 * (r + arcWidth / chartSize) / chartSize)
                        .attr("cy", Math.sin(dotDeg) * height * 1.8 * (r + arcWidth / chartSize) / chartSize)
                ...
                const thisSong = billboards.filter(({title}) => title === full_title)[0];
                const idMatch = {1:1, 2:2, 3:3, 4:8, 5:9, 6:4, 7:10, 8:11, 9:5, 10:12, 11:6, 12:13, 13:14, 14:7};
                const songId = idMatch[thisSong['id']];
                const unitMult = height * 1.8 * (r + arcWidth / chartSize) / chartSize;
                let posAdjustX = height * 0.016, posAdjustY = height * 0.017;
                switch (songId) {
                    case 9:
                        posAdjustX *= 0.95;
                ...
                if (selected > 0)
                    index.append("circle")
                        ...
                        .style("fill", selected === songId ? "rgba(128,240,255,0.7)" : "rgba(255,255,255,0.2)")
                        .style("stroke", selected === songId ? "white" : "rgba(255,255,255,0.5)");
                else 
                    index.append("circle")
                        .attr("class", `${full_title}-index-circle`)
                        .attr("r", height * 1.8 / 180)
                ...
                else if (songId === 4) { posAdjustX *= 1.5; posAdjustY = -height * 0.004; }
                else if (songId === 9) { posAdjustY = height * 0.002; }
                if (songId === 14) { posAdjustX *= 1.75; posAdjustY = -height * 0.004; }
                else if (songId === 12) { posAdjustX *= 0.9; posAdjustY = -height * 0.01; }
                else if (songId === 1) { posAdjustX *= 1.1; }
                else if (songId === 11) { posAdjustX *= 1.1; }
                else if (songId === 10) { posAdjustX *= 2.4; posAdjustY *= 1.35; }
                index.append("text")
                    .text(songId)
